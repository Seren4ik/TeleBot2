...
class ReplyKeyboardMarkup(base.TelegramObject):
    """
    This object represents a custom keyboards with reply options (see Introduction to bots for details and examples).
...
class KeyboardButton(base.TelegramObject):
    """
    This object represents one button of the reply keyboards.
...
class ReplyKeyboardRemove(base.TelegramObject):
    """
    Upon receiving a message with this object, Telegram clients will remove the current custom keyboards and display the default letter-keyboards. By default, custom keyboards are displayed until a new keyboards is sent by a bot. An exception is made for one-time keyboards that are hidden immediately after the user presses a button (see ReplyKeyboardMarkup).
